...
func HandleErrorResponse(message string, statusCode int) appctx.Response {
	return *appctx.NewResponse().WithCode(statusCode).WithMessage("ERROR").WithData(fiber.Map{"error": message})
}

func GetParameter(formData *multipart.Form, key string) (string, error) {
	values, ok := formData.Value[key]
	if !ok || len(values) == 0 {
